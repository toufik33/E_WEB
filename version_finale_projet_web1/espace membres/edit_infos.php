...
if(isset($_SESSION['username']))
{
	...
	if(isset($_POST['username'], $_POST['password'], $_POST['passverif'], $_POST['email'],$_POST['adresse'], $_POST['avatar']))
	{
			...
			if(strlen($_POST['password'])>=6)
			{
				...
				if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
				{
					...
					if($dn['nb']==0 or $_POST['username']==$_SESSION['username'])
					{
						...
						if(mysql_query('update users set username="'.$username.'", password="'.$password.'", email="'.$email.'", adresse="'.$adresse.'" , avatar="'.$avatar.'" where id="'.mysql_real_escape_string($_SESSION['userid']).'"'))
						{
...
<?php
	...
	if($form)
	{
		...
		if(isset($_POST['username'],$_POST['password'],$_POST['email'],$_POST['adresse']))
		{
			...
			$email = htmlentities($_POST['adresse'], ENT_QUOTES, 'UTF-8');
			$avatar = htmlentities($_POST['avatar'], ENT_QUOTES, 'UTF-8');
		}
		else
		{
			//Sinon, on affiche les donnes a partir de la base de donnee
			$dnn = mysql_fetch_array(mysql_query('select username,password,email,adresse,avatar from users where username="'.$_SESSION['username'].'"'));
			$username = htmlentities($dnn['username'], ENT_QUOTES, 'UTF-8');
			$password = htmlentities($dnn['password'], ENT_QUOTES, 'UTF-8');
			$email = htmlentities($dnn['email'], ENT_QUOTES, 'UTF-8');
			$adresse = htmlentities($dnn['adresse'], ENT_QUOTES, 'UTF-8');
...
<div class="content">
    <form action="edit_infos.php" method="post">
        ...
        <div class="center">
			...
			<label for="adresse">Adresse</label><input type="text" name="adresse" id="adresse" value="<?php echo $adresse; ?>" /><br />
